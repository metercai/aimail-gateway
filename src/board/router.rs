    ...
    fn mount(&self, router: Router) -> Router {
        ...
        let board_api = Router::new()
            .route(
                ...
                get(handlers::handle_get_task),
            )
            .route(
                "/api/v1/board/:board_id/tasks",
                get(handlers::handle_list_tasks),
            )
            .route(
                "/api/v1/board/:board_id/members",
                get(handlers::handle_list_members),
            )
            .route(
                "/api/v1/board/:board_id/status",
                get(handlers::handle_board_status),
            )
            .route(
                "/api/v1/board/:board_id/roles",
                get(handlers::handle_list_roles),
            )
            .route(
                "/api/v1/board/:board_id/task/:task_id/heartbeat",
                post(handlers::handle_post_heartbeat),
            )
