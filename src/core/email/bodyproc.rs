...
// ═══════════════════════════════════════════════════════════════
// Fixture-driven tests — verify pipeline on realistic emails
// ═══════════════════════════════════════════════════════════════
...
mod fixture_tests {
    ...
    #[test]
    fn test_reply_on_wrote() {
        // 2 layers. L0 has no sig (content only). L1 has Alice's sig.
        assert_fixture("reply-on-wrote", 2, false, true);
        // Verify L0 body is clean (no signature text leaked)
        let body = load_fixture("reply-on-wrote");
        let result = process_email_body(&body, false);
    ...
    #[test]
    fn test_forward_original_msg() {
        assert_fixture("forward-original-msg", 2, false, true);
    }

    #[test]
    fn test_nested_reply_forward() {
        // Bob's sig (bare name at end), Alice's `>` block has no explicit sig.
        // Manager X sig is in L2 (nested forward), captured via named closing.
        assert_fixture("nested-reply-forward", 3, true, false);
    }

    #[test]
    fn test_no_signature() {
        assert_fixture("no-signature", 1, false, false);
    }

    #[test]
    fn test_signature_rfc3676() {
        assert_fixture("signature-rfc3676", 1, true, false);
        let body = load_fixture("signature-rfc3676");
        let result = process_email_body(&body, false);
        assert!(result.signature.as_ref().unwrap().raw.contains("John Doe"));
    }

    #[test]
    fn test_disclaimer_footer() {
        // With `-- ` fixed, signature IS found
        let body = load_fixture("disclaimer-footer");
        let result = process_email_body(&body, false);
        // Signature extraction works on raw body. "CONFIDENTIAL" not in markers
        // but doesn't interfere with `-- ` detection.
        assert_fixture("disclaimer-footer", 1, true, false);
        let body = load_fixture("disclaimer-footer");
        let result = process_email_body(&body, false);
    ...
    #[test]
    fn test_gt_in_code() {
        assert_fixture("gt-in-code", 1, false, false);
        let body = load_fixture("gt-in-code");
        let result = process_email_body(&body, false);
